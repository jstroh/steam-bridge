...
function runWindowsSmokeHelperStaticChecks() {
  ...
  for (const expected of [
    ...
    "maximumTargetUnsynchronizedSampleCount",
    "targetDisplayToleranceHz",
  ...
  for (const expected of [
    ...
    "Get-PersistentReuseAnchoredWebCloseTarget",
  ...
  for (const expected of [
    ...
    "minimumModalTopInset",
    "minimumModalRightInset",
...
function runElectronSmokeActionStaticChecks() {
  ...
  for (const [label, source, expected] of [
    ...
    ["Electron smoke main", main, "await openAchievementUnlock()"],
    ["Electron smoke main", main, "await openPresenterAchievementUnlock()"],
    ["Electron smoke main", main, "await runAchievementUnlockSmoke(activeClient)"],
    ["Electron smoke main", main, "waitForEventCountAfter("],
    ["Electron smoke main", main, "clearConfirmedForProgress"],
    ["Electron smoke main", main, "Steam did not confirm the cleared achievement before unlock proof."],
...
assert.equal(typeof steamServer.buildSteamWebApiUrl, "function");
...
assert.equal(typeof steamServer.requestSteamWebApi, "function");
assert.equal(typeof steamServer.SteamPublisherSecretsClientRuntimeError, "function");
