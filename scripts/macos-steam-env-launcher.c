...
static char *default_target_for_launcher(const char *argv0) {
  ...
  if (strcmp(base_name, "SteamBridgeSmoke") == 0) {
    if (snprintf(sibling_target, sizeof(sibling_target), "%s/SteamBridgeSmoke.electron", directory) >=
        (int)sizeof(sibling_target)) {
      fprintf(stderr, "Sibling Electron target path is too long under: %s\n", directory);
      exit(2);
    }

    if (access(sibling_target, X_OK) == 0) {
      return copy_string(sibling_target);
    }
  }
  ...
  if (snprintf(
        target,
        sizeof(target),
        "%s/SteamBridgeSmoke.app/Contents/MacOS/SteamBridgeSmoke",
        directory
      ) >= (int)sizeof(target)) {
    fprintf(stderr, "Default Electron target path is too long under: %s\n", directory);
    exit(2);
  }

  return copy_string(target);
}
