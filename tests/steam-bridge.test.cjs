...
test("electron steam overlay direct wait fails known unavailable status before activating", async (t) => {
  setProcessPlatformForTest(t, "linux");
  const hostHandle = Buffer.from([4, 8, 15, 23]);
  let hostOpen = false;
  const fake = createFakeNative({
    attachNativeOverlayHostView(nativeWindowHandle) {
      hostOpen = true;
  ...
  const overlay = steam.overlay.createElectronSteamOverlay(window, {
    title: "Electron Direct Wait Unavailable Overlay",
    pollIntervalMs: 50
  });

  const status = overlay.getWebOpenStatus("https://store.steampowered.com/app/480/", { modal: true });
  assert.equal(status.canOpen, false);
  assert.equal(status.canWait, false);
  assert.equal(status.reason, "steam-unavailable");
  assert.equal(status.waitReason, "steam-unavailable");
  assert.equal(await overlay.openWebAndWaitIfAvailable(
    "https://store.steampowered.com/app/480/",
    { modal: true },
    { showTimeoutMs: 5, closeTimeoutMs: 5 }
  ), null);
  await assert.rejects(
    overlay.openWebAndWait("https://store.steampowered.com/app/480/", { modal: true }, {
      showTimeoutMs: 5,
      closeTimeoutMs: 5
    }),
  ...
  assert.deepEqual(steamWebOverlayCalls(fake), []);
  assert.deepEqual(fake.calls.filter((call) => call.method === "setNativeOverlayHostInputPassthrough"), []);
  assert.deepEqual(fake.calls.filter((call) => call.method === "setNativeOverlayHostOpacity"), []);

  overlay.close();
});

test("electron steam overlay direct wait waits for overlay readiness before activating", async (t) => {
  setProcessPlatformForTest(t, "linux");
  ...
  const overlay = steam.overlay.createElectronSteamOverlay(window, {
    title: "Electron Direct Wait Readiness Overlay",
    pollIntervalMs: 50
  });

  const status = overlay.getWebOpenStatus("https://store.steampowered.com/app/480/", { modal: true });
  assert.equal(status.canOpen, false);
  assert.equal(status.canWait, true);
  assert.equal(status.reason, "overlay-not-ready");

  const directOpen = overlay.openWebAndWaitIfAvailable(
    "https://store.steampowered.com/app/480/",
    { modal: true },
    { showTimeoutMs: 500, closeTimeoutMs: 500 }
  );
  assert.notEqual(directOpen, null);
...
test("electron steam overlay macOS global shortcut waits for overlay readiness before opening", async (t) => {
  setProcessPlatformForTest(t, "darwin");
  ...
  const overlay = steam.overlay.createElectronSteamOverlay(window, {
    title: "Electron macOS Shortcut Readiness Overlay",
    pollIntervalMs: 50
  });

  assert.equal(registerCount, 1);
  assert.equal(typeof registeredHandler, "function");
  assert.equal(overlay.getShortcutOpenStatus().reason, "overlay-not-ready");
  ...
  overlayEnabled = true;
  overlay.pump();
  await waitForNextSteamWebOverlayCall(fake, 0, [steam.STEAM_FRIENDS_OVERLAY_URL, true]);

  fake.callbacks.get(steam.SteamCallback.GameOverlayActivated)({ active: true });
  windowFocused = false;
  fake.callbacks.get(steam.SteamCallback.GameOverlayActivated)({ active: false });
  await waitForCondition(() => registerCount === 2, 500);

  assert.equal(typeof registeredHandler, "function");
  assert.equal(showCount >= 1, true);
  assert.equal(focusCount >= 1, true);
  assert.equal(invalidateCount >= 1, true);
  assert.equal(windowFocused, true);
  assert.deepEqual(steamWebOverlayCalls(fake), [
    { method: "activateOverlayToWebPage", args: [steam.STEAM_FRIENDS_OVERLAY_URL, true] }
  ]);
...
test("electron steam overlay manager exposes named open status helpers", async (t) => {
  const hostHandle = Buffer.from([2, 4, 6, 8]);
  let hostOpen = false;
  const fake = createFakeNative({
    attachNativeOverlayHostView(nativeWindowHandle) {
  ...
  const overlay = steam.overlay.createElectronSteamOverlay(window, {
    title: "Named Open Status Helpers"
  });
  t.after(() => overlay.close());

  const statusResult = { canOpen: true, canWait: true };
  const calls = [];
  overlay.getOpenStatus = (target) => {
    calls.push(target);
    return statusResult;
  ...
  assert.equal(overlay.getWebOpenStatus("https://example.test/", { modal: true }), statusResult);
  assert.equal(overlay.getStoreOpenStatus({ appId: 480, flag: steam.StoreFlag.AddToCart }), statusResult);
  assert.equal(overlay.getFriendsOpenStatus({ modal: false }), statusResult);
  assert.equal(overlay.getProfileOpenStatus({ steamId64: "76561198000000000" }), statusResult);
  assert.equal(overlay.getPlayersOpenStatus({ steamId64: "76561198000000000" }), statusResult);
  assert.equal(overlay.getCommunityOpenStatus({ appId: 480 }), statusResult);
  assert.equal(overlay.getStatsOpenStatus({ appId: 480, steamId64: "76561198000000000" }), statusResult);
  assert.equal(overlay.getAchievementsOpenStatus({ appId: 480 }), statusResult);
  assert.equal(
    overlay.getUserOpenStatus({ dialog: steam.UserDialog.Chat, steamId64: "76561198000000000" }),
    statusResult
  );
  assert.equal(
    overlay.getCheckoutOpenStatus({ transactionId: "123456789", returnUrl: "steam://run/480", modal: true }),
    statusResult
  );
  assert.equal(overlay.getDialogOpenStatus({ dialog: steam.Dialog.OfficialGameGroup, appId: 480 }), statusResult);

  assert.deepEqual(calls, [
    { modal: true, type: "web", url: "https://example.test/" },
    { appId: 480, flag: steam.StoreFlag.AddToCart, type: "store" },
    { modal: false, type: "friends" },
    { steamId64: "76561198000000000", type: "profile" },
    { steamId64: "76561198000000000", type: "players" },
    { appId: 480, type: "community" },
    { appId: 480, steamId64: "76561198000000000", type: "stats" },
    { appId: 480, type: "achievements" },
    { dialog: steam.UserDialog.Chat, steamId64: "76561198000000000", type: "user" },
    { transactionId: "123456789", returnUrl: "steam://run/480", modal: true, type: "checkout" },
    { dialog: steam.Dialog.OfficialGameGroup, appId: 480, type: "dialog" }
  ]);
});
...
test("electron steam overlay checkout IfAvailable waits through readiness and skips hard blockers", async (t) => {
  setProcessPlatformForTest(t, "linux");
  ...
  assert.equal(disabledCheckoutOperationStatus.canWait, true);
  assert.equal(disabledCheckoutOperationStatus.reason, "overlay-not-ready");
  assert.equal(disabledCheckoutOperationStatus.waitReason, undefined);
  ...
  const readinessCheckout = overlay.openCheckoutAndWaitIfAvailable(
    () => {
      readinessOperationRan = true;
      return "123456789";
    },
    { showTimeoutMs: 500, closeTimeoutMs: 500 }
  );
  await new Promise((resolve) => setTimeout(resolve, 80));
  assert.equal(readinessOperationRan, false);
  assert.deepEqual(steamWebOverlayCalls(fake), []);

  overlayEnabled = true;
  await waitForNextSteamWebOverlayCall(fake, 0, [
    "https://checkout.steampowered.com/checkout/approvetxn/123456789/",
    true
  ]);
  fake.callbacks.get(steam.SteamCallback.GameOverlayActivated)({ active: true });
  fake.callbacks.get(steam.SteamCallback.GameOverlayActivated)({ active: false });
  const readinessCheckoutResult = await readinessCheckout;
  assert.notEqual(readinessCheckoutResult, null);
  assert.equal(readinessOperationRan, true);
  assert.deepEqual(readinessCheckoutResult.targetSnapshot, {
    type: "checkout",
    hasTransactionId: true
  });
  ...
  await assert.rejects(
    overlay.withCheckoutPrepared(() => {
      steamStoppedPreparedOperationRan = true;
      return "prepared";
    }),
    (error) => {
      assert.match(error.message, /Steam is not running/);
  ...
  assert.equal(steamStoppedPreparedOperationRan, false);
  assert.equal(steamWebOverlayCalls(fake).length, checkoutCallsAfterReadiness);
