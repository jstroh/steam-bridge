...
| macOS Apple Silicon | Exact `338f203` signed smoke package passed 55/55 Steam route cases on Retina/Metal. RC80/RC85 focused actual-game receipts close the attached-child geometry, gesture, minimum-size, native-state, overlay lifecycle, GPU-recovery, Retina/1x visual-shape, and display-supervisor defects. Test-only RC89 on prerelease Electron isolated the upstream M152 cadence repair but is not releasable. The current signed/notarized/stapled candidate uses stable Electron 43.2.0; focused actual-game/baseline and the corrected slow-title-drag path pass while the one exact Chromium post-restore half-rate signature is an explicit receipt-visible product-owner exception. | Finish only still-unproven exact-candidate cases. Treat unreliable consecutive synthetic AppKit title drags as an instrumentation blocker, not a product pass; obtain reliable one-held-drag or independent evidence before release. Then run one final complete actual-game pass and the 55-route gate on the unchanged stable candidate. Lock-capable sleep is permanently excluded. |
