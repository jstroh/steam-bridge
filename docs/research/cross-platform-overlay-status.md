...
6. Confirm `callback:microtxn` includes a presenter snapshot if authorization
   ...
   artifacts where that callback lacks presenter diagnostics or lands outside
   the `openCheckoutAndWait(...)` lifecycle before checkout wait completion.
