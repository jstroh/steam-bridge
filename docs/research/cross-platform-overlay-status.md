...
| macOS Apple Silicon | Verified for Steam-launched managed web/store/Friends/dialog wait overlays and passive progress toast | The packaged macOS arm64 smoke app uses an in-bundle native launcher that preserves Steam overlay injection while setting `SteamAppId`, `SteamGameId`, and `SteamOverlayGameId` to App ID `480` before `exec`ing Electron. A 2026-06-29 strict `presenter-web-open-and-wait --web-modal true` run launched from Steam, emitted active and inactive overlay callbacks, completed `openAndWait(...)` only after close and presenter parking, kept the parked presenter transparent/click-through at `currentFps=0` with unchanged `pumpCount`, returned `SteamBridgeSmoke.electron` frontmost, and reported no crash evidence. Follow-up 2026-06-29 macOS runs verified the same close/back-to-app and `openAndWait(...)` completion-after-parking contract for `presenter-store-open-and-wait`, `presenter-friends-open-and-wait`, and `presenter-dialog-auto-open-and-wait --dialog OfficialGameGroup`. A focused macOS passive run at `/tmp/steam-bridge-macos-passive-toast-20260629-094107` verified `presenter-achievement-progress` with `callback:achievement-stored`, no modal activation, a passive transparent/click-through presenter, and no crash evidence. |
| macOS Apple Silicon matrix automation | Implemented; minimal live coverage exercised | `scripts/macos-overlay-matrix.sh` dry-runs or executes repeatable Steam-launched macOS helper cases. It installs or updates one stable non-Steam shortcut with the native launcher env-file flag, restarts Steam only when that shortcut is added or materially changed, and writes per-case launch state through the env file. It collects result/diagnostic logs for web/store/Friends/dialog wait routes, passive toast gates, synthetic checkout approval-route plumbing, and common presenter-backed web targets. A 2026-06-29 minimal run at `/tmp/steam-bridge-macos-overlay-matrix-minimal-20260629-093825` passed web/store/Friends/dialog close-and-park cases; the passive case exposed a verifier false negative that was fixed and then passed in the focused run above. Full macOS checkout and broader target coverage remains pending. |
