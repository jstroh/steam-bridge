...
| Windows x64 | Helper coverage implemented; live run blocked by Windows Smart App Control signing | The packaged Windows x64 smoke app includes `windows-electron-smoke.ps1` for direct, Steam-launched, verification, preflight, and shortcut launch-option checks. The helper accepts the same generic smoke action names used by Deck/macOS for web, store, Friends, dialog-equivalent, checkout, shortcut, and passive notification regression coverage, plus `-OverlayDisableDirectComposition 1` for an explicit Windows composition workaround comparison. The Windows package also includes `sign-windows-package.ps1`, which signs or audits all `.exe`, `.dll`, and `.node` files in the exact smoke bundle using an installed Authenticode certificate or PFX. A July 1, 2026 Windows 11 laptop test used a macOS-built Electron `43.0.0` package assembled from GitHub's `x86_64-pc-windows-msvc` prebuild and deployed it to `LAPTOP-9FD1HJGN`; Windows PowerShell 5.1 exposed and fixed a helper compatibility issue, then Smart App Control blocked `steam_bridge_native.win32-x64-msvc.node` before Steam could initialize. A fresh direct run on the same laptop with commit `47ae289` still failed before Steam initialization with `An Application Control policy has blocked this file` for the native addon. Code Integrity logged `Smart App Control Block Details` and policy `0283ac0f-fff1-49ae-ada1-8a933130cad6` because the native module did not meet Enterprise signing level requirements. A local self-signed test certificate could make Authenticode validation report `Valid`, but SAC still rejected the addon because local self-signing is not a trusted/reputable release path. Continue Windows overlay proof only with a properly trusted/reputable signed package or with Smart App Control explicitly disabled on the test machine. |
