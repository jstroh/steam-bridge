...
smoke app remains the focused X11 window, requires a delayed
`overlay:presenter-after-close` snapshot parked at passive idle
(`transparent=true`, `clickThrough=true`, `overlayActive=false`,
`currentFps=0`), and re-checks crash evidence after the close input.
