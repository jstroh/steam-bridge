...
`client.overlay.openSteamOverlay({ type: "friends", presenter })`, or the
lower-level `client.overlay.openFriendsOverlay({ presenter })`, which opens
...
`client.overlay.openSteamOverlay({ type: "achievements", appId, presenter })`,
or the lower-level `client.overlay.openAchievementsOverlay({ appId, presenter })`,
which opens
...
when that tool is available on the Deck. Focused passive-presenter Desktop runs
after `presenter-achievement-progress` did not open Steam overlay UI from
Shift+Tab or from the virtual controller Guide/Steam button. The full virtual
gamepad Guide run kept a single `gameoverlayui` attached to the main/native
process and briefly changed `overlayNeedsPresent` to `true`, but the screenshot
remained in the Electron app and no `GameOverlayActivated` callback was emitted.
That is evidence for the current overlay-toggle blocker, not a pass. Re-running
the same probe with `--overlay-game-id shortcut` made `gameoverlayui` attach to
the full non-Steam shortcut game ID instead of App ID `480`, but did not make
Shift+Tab or Guide open visible overlay UI.
...
the normal `overlay-open.png`. Add `--visual-toggle-input guide` to send a
controller Guide/Steam-button event through a temporary virtual gamepad, or
`--visual-toggle-input both` to compare Shift+Tab and Guide in one run. Add
`--overlay-game-id shortcut` to run the same visual probe while the wrapper sets
`SteamOverlayGameId` to the full non-Steam shortcut game ID; this is an
investigation switch for raw overlay close/back behavior, not a requirement for
the verified presenter web/Friends/checkout paths.
