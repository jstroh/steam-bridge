...
| Windows x64 | Helper and matrix coverage implemented; live run blocked by Windows Smart App Control signing | The packaged Windows x64 smoke app includes `windows-electron-smoke.ps1` for direct, Steam-launched, verification, preflight, stable smoke-env-file launch options, and shortcut launch-option checks. The helper accepts raw Windows baseline actions for web, store, Friends, and passive achievement notifications, plus the managed comparison action names used by Deck/macOS for dialog-equivalent, checkout, shortcut, and passive notification regression coverage. The package also includes `windows-overlay-matrix.ps1`, which runs preflight and then baseline, managed, or full Steam-launched suites with per-case artifacts; it can install/reuse one stable Steam shortcut and rewrite only a local smoke env file for each case. Preflight now writes both human logs and structured JSON with policy state, signatures, Zone.Identifier state, Steam process state, warnings, and matching Code Integrity events. Before any live Steam-launched case, the matrix runs a direct `none` native-load gate from the exact packaged app and requires real Steam initialization plus clean crash diagnostics, so Authenticode-looking packages that SAC/App Control still rejects fail under `00-preflight/native-load-gate`; failures also capture a post-gate preflight JSON snapshot after the blocked load attempt. Every live matrix case now requires clean Electron crash diagnostics so hidden renderer/GPU/native crashes fail the run. `-OverlayDisableDirectComposition 1` remains an explicit Windows composition workaround comparison rather than the default. A July 2, 2026 wrapper refresh found no safer maintained Windows Electron default than the current ordinary Steam-launched path; newer native-surface wrapper evidence is useful fallback research but still has open FPS/shutdown complaints, so it must pass the local Windows matrix before becoming a Windows default. The Windows package also includes `sign-windows-package.ps1`, which signs or audits all `.exe`, `.dll`, and `.node` files in the exact smoke bundle using an installed Authenticode certificate or PFX. A July 1, 2026 Windows 11 laptop test used a macOS-built Electron `43.0.0` package assembled from GitHub's `x86_64-pc-windows-msvc` prebuild and deployed it to `LAPTOP-9FD1HJGN`; Windows PowerShell 5.1 exposed and fixed a helper compatibility issue, then Smart App Control blocked `steam_bridge_native.win32-x64-msvc.node` before Steam could initialize. A fresh direct run on the same laptop with commit `47ae289` still failed before Steam initialization with `An Application Control policy has blocked this file` for the native addon. Code Integrity logged `Smart App Control Block Details` and policy `0283ac0f-fff1-49ae-ada1-8a933130cad6` because the native module did not meet Enterprise signing level requirements. A local self-signed test certificate could make Authenticode validation report `Valid`, but SAC still rejected the addon because local self-signing is not a trusted/reputable release path. Continue Windows overlay proof only with a properly trusted/reputable signed package or with Smart App Control explicitly disabled on the test machine. |
