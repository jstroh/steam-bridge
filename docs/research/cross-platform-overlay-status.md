...
| Desktop Mode high-level dialog equivalent router | Verified for known web-backed dialog names | The `presenter-dialog-auto --dialog Friends` action uses `steamOverlay.open({ type: "dialog", dialog: "Friends" })`, which now routes through the same presenter-backed Friends/chat web surface as `type: "friends"`. A 2026-06-28 Deck Desktop run emitted `callback:overlay-activated active=true`, captured the Friends/chat surface in the native overlay host, emitted `active=false` after the web close probe, returned focus to the Electron app, and used one `gameoverlayui` target attached to the app process with game ID `480`. Raw `presenter-dialog` still passes `route: "native"` and remains investigation-only. |
...
| `steamOverlay.open({ type: "dialog", dialog })` | For `Friends`, `Community`, `OfficialGameGroup`, `Stats`, and `Achievements`, the high-level router opens the verified presenter-backed web equivalent instead of raw `ActivateGameOverlay(...)`. A 2026-06-28 `presenter-dialog-auto --dialog Friends` Deck Desktop run proved the route through visible Friends/chat content, active/inactive callbacks, one overlay target, and clean return to the app. Pass `route: "native"` to intentionally exercise the raw dialog path. | Keeps the easy app-facing API on the Deck-verified route while preserving raw Desktop social/dialog investigation coverage. |
