...
| Steam Deck Game Mode | Verified for bounded managed readiness and compositor-native store lifecycle | The dedicated two-case contract passes Steam launch/injection, Deck and Big Picture identity, overlay readiness, passive managed X11/GLX presenter attachment at 0 FPS, compositor-native store `active=true`, Gamescope screenshots, exact SteamUI Escape `active=false`, app focus return, clean crash evidence, and zero-remnant cleanup. A fresh managed-web control still creates the host without an active callback or overlay target, so the Desktop managed-route matrix remains Desktop-only. |
...
| General web-page overlay | Not working from the Deck Game Mode smoke shortcut | `ActivateGameOverlayToWebPage` to a normal Steam web page was called successfully, but did not show a visible web overlay or produce `active=true`. A fresh 2026-07-15 managed presenter control reached the same boundary with an attached X11/GLX host and no overlay target; use the dedicated Game Mode raw-store contract instead of retrying this route. |
...
A fresh 2026-07-14 Windows-hosted Deck pass found three transport/session
requirements in that runner. Git Bash on NTFS cannot reliably represent Linux
execute bits, so the local package gate checks readable regular files and the
copy step restores execute permission on the app, packaged helper, Crashpad
handler, and Chromium sandbox. Direct SSH launches supply the active X11,
Xauthority, and user D-Bus environment. Same-shortcut relaunch also performs a
bounded exact-runtime cleanup before copying: it recognizes a retained process
whose executable was replaced and is marked deleted by Linux, then removes only
that smoke tree, its inhibitor, and overlay helpers whose target is missing,
zombie, or exited. It does not stop Steam or a live game target. State artifacts retain process
identity and redacted environment-key presence rather than command lines or
values. The corrected path passed direct packaged App ID `480` verification and
two consecutive retained focused launches without a Steam restart.

The fresh 2026-07-15 mode pass now separates the proof contracts explicitly.
Desktop Mode passes the strict 26-case managed matrix with 52 screenshots and a
clean receipt. Game Mode passes `--mode game --suite game`: managed presenter
readiness stays passive at 0 FPS with isolation and zero timing, then raw store
activation renders through SteamUI, emits active/inactive callbacks, captures
through `gamescopectl screenshot`, closes through exact Escape on SteamUI's
Xwayland display, restores the focused fullscreen app, and exits crash-free.
The Game receipt records zero smoke, inhibitor, overlay, or Plasma processes
while Steam remains running. Spectacle without Plasma, Guide navigation, and an
unregistered virtual-controller B button are rejected Game Mode paths. The
same pass reconfirmed that presenter-backed web does not activate or create an
overlay target in Game Mode, so the runner refuses to execute Desktop managed
suites there.
...
| `presenter-achievement-progress` / `presenter-achievement-unlock` | Reuses one attached passive presenter, relies on automatic passive notification priming from the managed Electron overlay, and captures visible Steam achievement-progress and achievement-unlock toasts over the app with a single overlay target. The unlock action clears and re-unlocks the selected public test achievement so repeated smoke runs can exercise a real unlock notification. Fresh 2026-07-14 focused Desktop runs completed the required achievement callbacks, visibly rendered both public toasts, remained non-modal, and found the transparent/click-through presenter parked at `currentFps=0` without a needs-present transition. Their state-driven completion event records `renderPath="steam-overlay-target"`; platforms that emit the transition record `needs-present` instead. | Passive notification/toast behavior works without app-level priming, without making the native host interactive, without a fixed high-FPS preparation window, and without requiring a modal overlay activation callback or a Deck-specific needs-present transition. |
...
A fresh 2026-07-15 close-input comparison removed the Deck runner's fixed
percentage target. For ordinary presenter-backed web surfaces it captures the
physical display, detects the visible Steam X from its diagonal contrast inside
the native host header, clicks the detected glyph once, and still requires the
inactive callback, app focus, parking, stable pump count, and clean crash
evidence. Store `openAndWait` is a route-specific exception on current
Deck/KWin: two detected-X attempts did not deactivate it, one XTest attempt
activated KWin Overview, and device-level input at the detected point also did
not close it. Refocusing the smoke app and using Steam Bridge's managed
Shift+Tab pass-through closed the same retained store-wait surface and passed
the complete wait-helper lifecycle. The matrix uses that toggle for this case;
do not add more clicks or coordinate tuning.
...
The inverse is also not a passive-notification failure. Current Deck Steam can
render achievement progress and unlock toasts through its attached overlay
target without changing `BOverlayNeedsPresent()`. The notification verifier
therefore waits for the action's accepted Steam callbacks and a state-driven
parked passive presenter, records whether a needs-present transition occurred,
and records `needs-present` or `steam-overlay-target` as the observed render
path. A timeout remains a failure guardrail; increasing it does not turn the
absent transition into stronger evidence.
