...
app.whenReady().then(() => {
  ...
  console.error("Steam startup failed:", error);
  app.quit();
});
...
action definition generated by `steam-bridge-input generate` (see
[Steam Input](steam-input.md)):
...
for a newer frame, rather than replaying a pre-blur press. Compare
`steamActions.sequence`; the outer sample's own `sequence` numbers renderer
input frames, not Steam action frames, and is not a substitute.
...
The call can also throw synchronously, for example on a closed session or a
malformed descriptor. Treat a synchronous throw exactly like a rejection: a
`NativeOverlaySharedTextureCopyError` follows its `producerReleaseSafe` flag,
and any other error fails closed.
...
`createNativeWindow({ continuousPresent: true })` is useful when
capture/streaming requires repeated presentation of retained content even
without new Electron damage. It defaults to `false`. It is not an FPS repair
switch and must not add another application present loop.
